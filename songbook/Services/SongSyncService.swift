...
actor SongSyncService {
    ...
    init(dataManager: DataManager = .shared,
         ...
         downloader: ResourceDownloader? = nil) {
        self.dataManager = dataManager
        self.networkMonitor = networkMonitor
        self.downloader = downloader ?? ResourceDownloader(
            networkMonitor: networkMonitor,
            basePDFURL: dataManager.pdfDownloadURL,
            baseMP3URL: dataManager.mp3DownloadURL
        )
    ...
    func sync() async {
        ...
        do {
            ...
            try dataManager.populate(inactiveContainer, with: songs, favorites: favorites)
        } catch {
    ...
    func verifyIntegrity() async {
        ...
        if missingPDFs > 0 || missingMP3s > 0 {
            Self.logger.info("Queued downloads: \(missingPDFs) PDFs, \(missingMP3s) MP3s")
            await downloader.processQueue()
        } else {
            Self.logger.info("All resources present")
        }
    }
